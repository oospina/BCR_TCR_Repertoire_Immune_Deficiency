...
# or pre-collapsing
match_core_props <- c("locus", 
                      #"isotype", # Not in Marta's data
                      "v_call", "j_call",
...
match_extr_props <- c( "sequence", "sequence_alignment",
                       ...
                       #"repertoire_id" # Not in Marta's data
                       "cdr3" # Needed by Marta to query database
                       )
...
#tab_file = './data/prep/1index12_IGH_prepared.tsv'
...
#rep.collapsed <- collapseCloseSequences( rep, thr_count=1, discard_low_copy=T ) %>% # Needed to lower count threshold as sample 1index14 did not have enough sequences
                 filter( productive == TRUE )

# write out the results into a new database
#write_tsv(rep.collapsed, path=newtab_file, quote_escape=F) # DEPRECATED ARG
if(nrow(rep.collapsed) > 0){
  write_tsv(rep.collapsed, path=newtab_file, quote='none')
} else{
  cat(paste0('NO SEQUENCES AVAILABLE IN THIS SAMPLE ( ', stringr::str_extract(tab_file, '[0-9]index[0-9]+_[IGHTRB]{3}'), ' )\n'))
}
