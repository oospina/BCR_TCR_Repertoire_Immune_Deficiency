...
dnapars_bin = '/home/oospina1/.conda/envs/btcr_tools/bin/dnapars'
logging      = TRUE
                                 ...
                                 rm_temp=FALSE, verbose=FALSE)
{
  ...
  if( nrow(clone_in@data) > 1 )
  {
    ...
    V(graph)$v_gene  <- paste0( clone_in@clone, "_", V(graph)$v_gene  )
  ...
  else
  {
    ...
    graph <- make_empty_graph( directed=F ) %>%
             add_vertices(1,
                          ...
                          #isotype   = clone_in@data$isotype[1], # No isotype data in Marta's data
                          #repertoire_id = clone_in@data$repertoire_id[1], # Column not present in Marta's data
...
# ---- group by clones
clones <- seqdb %>% 
  mutate(v_gene=alakazam::getGene(seqdb$v_call)) %>% # Add V-gene for graph plotting 
  group_by(clone_id) %>%
